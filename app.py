...
for d in DIAMETERS:
    ...
    with st.expander(f"Diameter {d} mm"):
        ...
        # Collect valid rows (length>0 and quantity>0)
        lengths_list = []
        for r in rows:
...
if st.button("Run Optimization"):
    ...
    for d in DIAMETERS:
        if d in data:
            ...
            for r in st.session_state[f"rows_{d}"]:
                if r["Length"]>0 and r["Quantity"]>0:
                    counter[r["Length"]] += r["Quantity"]
            for length, qty in counter.items():
                w = length * qty * weight_per_meter(d)
                mainbar_records.append({"Diameter": d, "Length": length, "Quantity": qty, "Weight (kg)": w})
    mainbar_df = pd.DataFrame(mainbar_records)
    mainbar_df.sort_values(by=["Diameter","Length"], inplace=True)
    ...
    for d, lengths in data.items():
        ...
        # Cutting instructions aggregation
    ...
    st.markdown("### MainBar")
    st.dataframe(mainbar_df.style.format({"Weight (kg)":"{:.2f}"}))
