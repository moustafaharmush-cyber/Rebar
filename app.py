...
# =========================
# Prepare MainBar Table
# =========================
def prepare_mainbar_table(session_rows):
    aggregated = defaultdict(lambda: {"Quantity":0, "Weight":0})
    for d, rows in session_rows.items():
        for r in rows:
            length = r["Length"]
            qty = r["Quantity"]
            if length > 0 and qty > 0:
                ...
                aggregated[key]["Weight"] += qty * length * weight_per_meter(d)

    data = []
    total_weight = 0
    for (d,length), info in sorted(aggregated.items()):
        data.append({"Diameter": d,
                     "Length (m)": length,
    ...
    return df, total_weight

# =========================
# PDF Generator
# =========================
def generate_pdf(df_main, total_main_weight, waste_df, purchase_df, cutting_instr_df, price):
    ...
    col_widths = [25, 25, 25, 35]
    headers = ["Diameter", "Length (m)", "Quantity", "Weight (kg)"]
    for i, h in enumerate(headers):
        pdf.cell(col_widths[i], 8, h, border=1, align="C")
    pdf.ln()
    for _, row in df_main.iterrows():
        pdf.cell(col_widths[0], 8, f"{int(row['Diameter'])} mm", border=1, align="C")
        pdf.cell(col_widths[1], 8, f"{row['Length (m)']:.2f}", border=1, align="C")
        pdf.cell(col_widths[2], 8, f"{row['Quantity']}", border=1, align="C")
        pdf.cell(col_widths[3], 8, f"{row['Weight (kg)']:.2f}", border=1, align="C")
        pdf.ln()
    pdf.set_font("Arial", 'B', 8)
    pdf.cell(col_widths[0]+col_widths[1]+col_widths[2], 8, "TOTAL", border=1, align="C")
    pdf.cell(col_widths[3], 8, f"{total_main_weight:.2f}", border=1, align="C")
    pdf.ln(10)
    ...
    pdf.set_font("Arial", '', 8)
    col_waste = [25,35,25,35]
    headers_waste = ["Diameter","Waste Length (m)","Number of Bars","Waste Weight (kg)"]
    for i,h in enumerate(headers_waste):
        pdf.cell(col_waste[i],8,h,border=1,align="C")
    pdf.ln()
    total_waste_weight = 0
    for _, row in waste_df.iterrows():
        pdf.cell(col_waste[0],8,f"{int(row['Diameter'])} mm",border=1,align="C")
        pdf.cell(col_waste[1],8,f"{row['Waste Length (m)']:.2f}",border=1,align="C")
        pdf.cell(col_waste[2],8,f"{int(row['Number of Bars'])}",border=1,align="C")
        pdf.cell(col_waste[3],8,f"{row['Waste Weight (kg)']:.2f}",border=1,align="C")
        pdf.ln()
        total_waste_weight += row['Waste Weight (kg)']
    pdf.set_font("Arial",'B',8)
    pdf.cell(col_waste[0]+col_waste[1]+col_waste[2],8,"TOTAL",border=1,align="C")
    pdf.cell(col_waste[3],8,f"{total_waste_weight:.2f}",border=1,align="C")
    pdf.ln(10)
    ...
    pdf.set_font("Arial",'',8)
    col_purchase=[25,25,35,35]
    headers_purchase=["Diameter","Number of Bars","Weight (kg)","Cost ($)"]
    for i,h in enumerate(headers_purchase):
        pdf.cell(col_purchase[i],8,h,border=1,align="C")
    pdf.ln()
    total_weight_purchase=0
    total_cost=0
    for _,row in purchase_df.iterrows():
        pdf.cell(col_purchase[0],8,f"{int(row['Diameter'])} mm",border=1,align="C")
        pdf.cell(col_purchase[1],8,f"{int(row['Bars'])}",border=1,align="C")
        pdf.cell(col_purchase[2],8,f"{row['Weight (kg)']:.2f}",border=1,align="C")
        pdf.cell(col_purchase[3],8,f"{row['Cost']:.2f}",border=1,align="C")
        pdf.ln()
        total_weight_purchase+=row['Weight (kg)']
        total_cost+=row['Cost']
    pdf.set_font("Arial",'B',8)
    pdf.cell(col_purchase[0]+col_purchase[1],8,"TOTAL",border=1,align="C")
    pdf.cell(col_purchase[2],8,f"{total_weight_purchase:.2f}",border=1,align="C")
    pdf.cell(col_purchase[3],8,f"{total_cost:.2f}",border=1,align="C")
    pdf.ln(10)
...
for d in DIAMETERS:
    with st.expander(f"Diameter {d} mm"):
        ...
        # Prepare lengths list for optimization (only non-zero)
        lengths_list=[]
        for r in rows:
            if r["Length"]>0 and r["Quantity"]>0:
                lengths_list.extend([r["Length"]]*r["Quantity"])
        if lengths_list:
            data_lengths[d]=lengths_list

# Display MainBar Table
mainbar_df, total_main_weight = prepare_mainbar_table({d: st.session_state[f"rows_{d}"] for d in DIAMETERS})
st.markdown("### MainBar (Input Bars)")
st.dataframe(mainbar_df.style.format({"Length (m)":"{:.2f}","Weight (kg)":"{:.2f}"}))
st.markdown(f"**Total Weight: {total_main_weight:.2f} kg**")
...
if st.button("Run Optimization"):
    ...
    for d,lengths in data_lengths.items():
        ...
        for bar in solution:
            bar_total=sum(bar)
            bar_waste=BAR_LENGTH-bar_total
            if bar_waste>0:
    ...
    df_waste=[]
    for (d,waste_len),info in waste_dict.items():
        df_waste.append([d,waste_len,info["count"],info["weight"]])
    waste_df=pd.DataFrame(df_waste,columns=["Diameter","Waste Length (m)","Number of Bars","Waste Weight (kg)"])
    ...
    # Generate PDF
    pdf_file=generate_pdf(mainbar_df,total_main_weight,waste_df,purchase_df,cutting_instr_df,price)
